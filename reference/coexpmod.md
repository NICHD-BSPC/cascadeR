...
UI returns sidebar/main panel UI elements for coexpression plot Server
returns reactive expression containing selected points from the
coexpression plot
...
ui <- shiny::fluidPage(
  shiny::sidebarPanel(coexpressionPlotUI("coexpression", "sidebar")),
  shiny::mainPanel(
    shiny::tabsetPanel(coexpressionPlotUI("coexpression", "main")),
    shiny::verbatimTextOutput("selected")
  )
)

server <- function(input, output, session) {
  selected <- coexpressionPlotServer(
    "coexpression",
