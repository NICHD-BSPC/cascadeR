...
``` r
clustSummaryUI(id, panel)

clustSummaryServer(id, obj, filtered, args, reload_global, config)
```
...
- args:
  ...
  reactive list with elements, 'grp_by' for grouping variable and
...
UI returns sidebar/main panel UI elements for the metadata viewer Server
called for the side effect of rendering metadata summary outputs.
