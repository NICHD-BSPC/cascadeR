...
data. Here is an overview of features and logical flow used in the app.
...
- Compatible with both Seurat objects (.Rds files) and AnnData objects
  (.h5ad files) from scanpy
...
- Cell Markers:
  - Three types of marker tables:
    - Cluster Markers: Genes that define each cluster (from
      FindAllMarkers/rank_genes_groups)
...
Cascade is built as a modular Shiny application with a central data
store and reactive programming model, allowing for interactive
exploration of single-cell and spatial transcriptomics data.

## Initialization and Configuration

### App Startup

- The app begins with the
  [`run_cascade()`](https://nichd-bspc.github.io/cascadeR/reference/run_cascade.md)
  function, which is the main entry point
- Configuration is loaded from a YAML file using
  [`get_config()`](https://nichd-bspc.github.io/cascadeR/reference/get_config.md)
...
- If credentials are provided, the app uses `shinymanager` to handle
  user authentication
...
- App determines the object type (Seurat or AnnData) based on file
  extension
- Data is loaded using either
  [`readRDS()`](https://rdrr.io/r/base/readRDS.html) for Seurat objects
  or `read_h5ad()` for AnnData
...
- Central Data Store
  - The app_object reactive values object serves as the central data
    store
  - Contains the loaded object, metadata, marker tables, and other
    shared data
...
- Module Interactions
  ...
  - Cell Embeddings → Filtering:
    - Selected points from UMAP/spatial plots are stored in
      `app_object$selected_points`
    - Can be used as filters in the subset module
  - Marker Tables → Gene Selection:
    - Selected genes from marker tables are passed to the gene
      scratchpad
...
- Summary Module
  ...
  - Displays cell counts (raw and filtered if QC information is
    available)
...
- Cell Embeddings Module
  ...
  - Allows point selection, which is stored in
    `app_object$selected_points`
...
- Metadata Viewer Module
  - Provides multiple views of cell metadata:
    ...
    - Spatial Feature Plot: Visualization of metadata on spatial
      coordinates
...
- Cluster Tree Module
  ...
  - Visualizes hierarchical relationships between clusters using
    `clustree`
...
- Global Refresh
  - The “Reload” button in the global settings triggers
    `reload_global()`
...
- Configuration System
  ...
  - Configuration is passed to modules through the config reactive
    function
